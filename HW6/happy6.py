...
#creates a dictonary of countries, pop per million and GDP per capita and happiness, removes any country under a population of 100 million and prints it out
...
def read_gdp_data(happy_dict):
    ...
    print(heading[0]+','+heading[1]+','+heading[2]+','+'Happiness')#prints out the heading in the proper format
    for line in infile: #loops for every line in file
        line = line.strip() #removes empty space from each line
        line = line.split('\t')#splits the lines at the ,
        ...
        if country in happy_dict:#checks if country is in happiness index
            if float(mil_pop) >= 100:#checks if counties population is more than 100 million 
                print(country+','+mil_pop+','+gdp_cap+','+str(happy_dict[country]))#prints out the formated string in every loop
