...
#finds the avarage temprature given a variable number of years for every year up until the given year
...
for line2 in range(k, len(temp_list) - k):#loops for the legnth of the temp list minus the number given for years 
    ave = sum(temp_list[line2 - k:line2+k+1]) / (2*k+1)#finds the average value of the temprature using the given tempratures      
    year = 1880 + line2#adds the given number for years to the bace year
    print(str(year)+','+str( "{:.4f}".format(ave)))#prints out the year and the average in a nice to read format for every year given
#print(line)  
